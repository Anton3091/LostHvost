...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  return (
    <div className="profile-view w-full max-w-4xl mx-auto p-4 sm:p-6">
      {/* Profile Header Bar */}
      ...
      <div className="profile-settings bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden text-sm">
        <div className="profile-settings-push px-5 py-4 border-b border-slate-100 flex items-center justify-between">
      ...
      {/* TAB 1: All Ads */}
      {activeTab === 'ads' && (
        <div className="profile-ads-list space-y-4">
          {userAdsLoading ? (
            ...
            allAds.map(ad => {
              ...
              return (
                <div
                  key={ad.id}
                  className={`profile-ad-card bg-white border border-slate-200 rounded-3xl overflow-hidden transition ${!isActive ? 'profile-ad-card--inactive' : ''}`}
                >
                  <div className="profile-ad-summary flex flex-row items-start space-x-4 flex-1 w-full">
                    <div className="shrink-0">
                      <img
                        src={ad.photos[0] || ''}
                        alt="Фото"
                        className="profile-ad-photo rounded-2xl object-cover border border-slate-100"
                      />
                    </div>
                    
                    <div className="profile-ad-copy flex-1 min-w-0">
                      <span className={`profile-ad-status ${
                        isActive
                          ? 'profile-ad-status--active'
                          : ad.status === 'rejected'
                            ? 'profile-ad-status--rejected'
                            : 'profile-ad-status--neutral'
                      }`}>
                  ...
                  <div className="profile-ad-actions w-full shrink-0 flex items-center justify-end">
                    {isActive ? (
                      <button
                        onClick={() => onUnpublishAd(ad.id)}
                        className="profile-ad-action profile-ad-action--danger w-full text-xs transition cursor-pointer flex items-center justify-center space-x-1.5"
                      >
                      ...
                      <button
                        ...
                        className="profile-ad-action profile-ad-action--neutral w-full text-xs transition cursor-pointer flex items-center justify-center space-x-1.5 disabled:opacity-60 disabled:cursor-wait"
                      >
                      ...
                      <button
                        ...
                        className="profile-ad-action profile-ad-action--warning w-full text-xs transition cursor-pointer flex items-center justify-center space-x-1.5"
                      >
                        <Repeat className="w-4 h-4" />
                        <span>Изменить объявление</span>
                      </button>
                    ) : (
                      <span className="profile-ad-pending w-full text-center text-xs text-slate-400">Проверяем объявление…</span>
                    )}
                  </div>
                </div>
              );
