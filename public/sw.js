const CACHE = 'losthvost-shell-v3';
self.addEventListener('install', event => event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(['/', '/manifest.webmanifest', '/losthvost.png'])).then(() => self.skipWaiting())));
